...
def figure(runs, args):
  ...
  if len(axes.shape) > 1:
    for i in range(len(axes)):
      axes[i,-1].xaxis.get_offset_text().set_visible(True)
  else:
    axes[-1].xaxis.get_offset_text().set_visible(True)
  #if args.xlim:
...
def main(args):
  ...
  atari_runs = []
  dmc_runs = []
  if len(args.atari_tasks) != 0:
    atari_runs = atari_load_experiments(args)
    atari_runs += atari_load_dreamers(args)
  if len(args.dmc_tasks) != 0:
    dmc_runs = dmc_load_experiments(args)
    dmc_runs += dmc_load_dreamers(args)
  runs = dmc_runs + atari_runs
